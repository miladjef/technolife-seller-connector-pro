...
class TLSCP_Sync {
    ...
    public function sync_price($product_id) {
        $seller_item_code = get_post_meta($product_id, '_tlscp_seller_item_code', true);
        if (!$seller_item_code) {
            return $this->product_error($product_id, 'کد تنوع فروشنده برای محصول تنظیم نشده است.');
        }
    ...
    public function sync_inventory($product_id) {
        ...
        $stock_quantity = $product->managing_stock() ? (int) $product->get_stock_quantity() : ($product->is_in_stock() ? 999 : 0);
        ...
        $sales_code = get_post_meta($product_id, '_tlscp_sales_code', true);
        $body = array(
            'available' => $available,
            'leaveTime' => absint($opts['leave_time']),
            'maxBuyPerOrder' => absint($opts['max_buy_per_order']),
        );
        if ($sales_code !== '') {
        ...
        if ($result['success']) {
            ...
            if ($opts['auto_hide_zero_stock'] === 'yes') {
                if ($available <= 0) {
                    $this->api->hide_item($seller_item_code);
                    update_post_meta($product_id, '_tlscp_hidden', 'yes');
                } else {
                    $this->api->show_item($seller_item_code);
                    update_post_meta($product_id, '_tlscp_hidden', 'no');
                }
            }
        } else {
            update_post_meta($product_id, '_tlscp_last_error', $result['message']);
    ...
    public function sync_all_connected($price = true, $inventory = true, $limit = 50) {
        ...
        foreach ($query->posts as $product_id) {
            ...
            if ($inventory) { $result2 = $this->sync_inventory($product_id); $result['success'] = !empty($result['success']) && !empty($result2['success']); }
    ...
    public function get_remote_item_info($product_id) {
        $product_code = get_post_meta($product_id, '_tlscp_product_code', true);
        $seller_item_code = get_post_meta($product_id, '_tlscp_seller_item_code', true);
